      ...
      <div className="container flex gap-10">
        <div className="flex gap-16">
          <div className="flex flex-col">
            {tabs.map((tab, index) => (
              <div
                className={`w-[110px] h-[110px] flex justify-center items-center text-base text-white bg-[#333333] border border-b-white cursor-pointer font-semibold ${index ? 'bg-orangeLight' : }`}
                key={index}
                onClick={() => {
                  if (index) {
                    console.log(index)
